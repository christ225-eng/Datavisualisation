...
if reponse.status_code == 200:
    ...
    elements_jeux = soup.find_all("li", attrs={"class": "search-result-item"})
    ...
    with open("jeux_nintendo.csv", "w", newline="", encoding="utf-8") as fichier:
        ...
        for jeu in elements_jeux[:15]:
            balise_titre = jeu.find("h3")
            balise_prix = jeu.find("span", attrs={"class": "price"})
            
            if balise_titre:
                titre = balise_titre.get_text(strip=True)
                prix = balise_prix.get_text(strip=True) if balise_prix else "N/A"
                writer.writerow([titre, prix, "Switch"])
                
        time.sleep(1)
        
    print("Scraping terminé ! Le fichier jeux_nintendo.csv est prêt.")
